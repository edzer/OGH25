...
points = [Point(np.random.uniform(bbox[0], bbox[2]), np.random.uniform(bbox[1], bbox[3])) for _ in range(20)]
# gdf = gpd.GeoDataFrame(geometry=points, crs="EPSG:4326")
e = r.xvec.extract_points(points, x_coords="lon", y_coords="lat", name="")
e.name = "L7_ETMs"
e
```
...
More examples are found in Ch 7 of "Spatial Data science: With Applications in R and Python", online found [here](https://r-spatial.org/python/07-Introsf.html#package-stars).